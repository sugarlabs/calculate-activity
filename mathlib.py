...
#        _logger.debug('len(digits) %d, exp: %d, int_len: %d, disp_exp: %d, dot_pos: %d', len(digits), exp, int_len, disp_exp, dot_pos)
    ...
    def format_number(self, n):
        if isinstance(n, types.BooleanType):
            if n:
                return 'True'
            else:
                return 'False'
            ...
            return n
        elif isinstance(n, types.UnicodeType):
            return n
        elif isinstance(n, types.NoneType):
            return _('Undefined')
        elif isinstance(n, types.IntType):
            n = self.d(n)
        elif isinstance(n, types.FloatType):
            n = self.d(n)
        elif isinstance(n, types.LongType):
            n = self.d(n)
    ...
    def is_int(self, n):
        if isinstance(n, types.IntType) or isinstance(n, types.LongType):
            return True
